...
target = """                const targetCrit = aData.baseCrit + (aData.growthCrit * currentLv)
                return (
                  <div key="active-animus" style={{ padding: 16, background: 'rgba(255,255,255,0.05)', borderRadius: 8, border: '1px solid ' + colors.border }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                      <div style={{ fontFamily: 'var(--font-title)', fontSize: 16, color: '#fff', fontWeight: 800 }}>
                        {aData.name} Lv.{currentLv}
                      </div>
                      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 13, color: '#44ff88', padding: '4px 8px', background: 'rgba(68, 255, 136, 0.1)', borderRadius: 4 }}>
                        Active Spirit
                      </div>
                    </div>
                    <div style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
                      ...
                      <div style={{ flex: 3, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, fontSize: 14, fontFamily: 'var(--font-mono)' }}>
                    ...
                    <div style={{ fontSize: 13, fontFamily: 'var(--font-mono)', color: '#aaa', fontStyle: 'italic', background: 'rgba(0,0,0,0.3)', padding: 12, borderRadius: 6, borderLeft: '4px solid #888' }}>
                      {aData.description}
                    </div>
                  </div>
                )
...
if target in content:
    content = content.replace(target, target + showcase_injection)
    with open('src/screens/Ascension.jsx', 'w', encoding='utf-8') as f:
        f.write(content)
    print("Injected showcase successfully!")
else:
    print("Failed to find target in Ascension.jsx")
