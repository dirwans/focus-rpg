      ...
      {loading && !archonData ? (
        ...
        <>
                    ...
                    return (
                      <div key={c.username} className={`glass-panel cyber-panel panel-${activeRace}`} style={{ border: `1px solid ${isMyVote ? '#f5a623' : 'rgba(0, 229, 255, 0.2)'}`, padding: 12, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        ...
                        {canVote ? (
                          ...
                          <span style={{ fontSize: 13, color: '#ff4466', fontWeight: 'bold' }}>Lv.10 Req</span>
                        ) : null}
                      </div>
                    )
                  })}
          ...
          {(() => {
            ...
            const currentRank = isOwnRace ? player.pvpRank : null
            return (
              <div style={{ padding: '16px 16px 8px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                  <h3 style={{ margin: 0, color: '#f5a623', fontSize: 16, fontFamily: 'var(--font-title)' }}>🏷️ TITLE DATABASE</h3>
                  {isOwnRace && (
                    <span style={{ fontSize: 13, color: currentRank ? '#00ff88' : '#7ec8e3', fontFamily: 'var(--font-mono)' }}>
                      {currentRank ? `PvP Rank #${currentRank}` : 'Belum masuk Top 3'}
                    </span>
                  )}
                </div>
                <div style={{ color: '#7ec8e3', fontSize: 11, fontFamily: 'var(--font-mono)', marginBottom: 10 }}>
                  Otomatis berdasarkan Peringkat PvP #1-3 per bangsa — bukan permanen, hilang seketika jika turun peringkat.
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 16 }}>
                  {raceTitles.map((tt) => {
                    const isActive = currentRank === tt.pvpRank
                    return (
                      <div key={tt.id} className={`glass-panel cyber-panel panel-${activeRace}`} style={{ padding: 12, borderRadius: 8, display: 'flex', alignItems: 'center', gap: 12, opacity: isActive ? 1 : 0.5 }}>
                        <div style={{ fontSize: 28 }}>{isActive ? '🏅' : '🔒'}</div>
                        <div style={{ flex: 1 }}>
                          <div style={{ color: isActive ? '#00ff88' : '#f5a623', fontSize: 14, fontWeight: 'bold', fontFamily: 'var(--font-title)' }}>
                            {tt.name} {isActive && '· ACTIVE'}
                          </div>
                          ...
                          <div style={{ color: '#7ec8e3', fontSize: 11, fontFamily: 'var(--font-mono)' }}>
                            Syarat: Peringkat PvP #{tt.pvpRank} bangsa ini
                          </div>
                        </div>
                      </div>
                    )
...
const styles = {
  ...
  rankSub: { fontFamily: 'var(--font-body)', fontSize: 13, color: '#7ec8e3', marginTop: 2, fontWeight: 700 },
  ...
  rankNum: { fontFamily: 'var(--font-mono)', fontSize: 13, color: '#7ec8e3', width: 20, textAlign: 'center', fontWeight: 800 },
  ...
  flag: { position: 'absolute', bottom: -4, right: -4, fontSize: 13 },
  rankName: { fontFamily: 'var(--font-body)', fontSize: 13, color: '#e0f4ff', fontWeight: 700 },
  rankBadge: { fontFamily: 'var(--font-title)', fontSize: 13, color: '#7ec8e3', marginTop: 2, fontWeight: 800 },
  rankTime: { fontFamily: 'var(--font-mono)', fontSize: 13, color: '#00c8ff', fontWeight: 800 },
  rankMin: { fontFamily: 'var(--font-mono)', fontSize: 13, color: '#7ec8e3', fontWeight: 700 },
  ...
  myRankHandle: { fontFamily: 'var(--font-body)', fontSize: 13, color: '#7ec8e3', marginTop: 2, fontWeight: 700 },
}
