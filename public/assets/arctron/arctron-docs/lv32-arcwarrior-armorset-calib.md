# Kitab Sakti Kalibrasi Mecha Arctron Warrior (LV32 Set)

> Auto-generated from Arctron Dressing Room on 7/16/2026, 7:31:50 PM

## Master Coordinate Presets

```javascript
// GEAR_POINTS presets for GearOverlay.jsx:
const GEAR_POINTS = {
  armor:  [{ x: 0.002, y: 0.056, ax: 0.51, ay: 0.18, size: 0.457, rot: 0, z: 1 }],
  pants:  [{ x: 0.003, y: 0.404, ax: 0.5, ay: 0.22, size: 0.368, rot: 0, z: 2 }],
  boots: [
    { x: 0.139, y: 0.642, ax: 0.5, ay: 0.15, size: 0.414, rot: -4, z: 3, scaleX: 0.935, scaleY: 1.150, label: 'boot_l', splitSuffix: '_l' },
    { x: -0.149, y: 0.646, ax: 0.5, ay: 0.15, size: 0.444, rot: 0, z: 3, scaleX: 0.870, scaleY: 1.060, label: 'boot_r', splitSuffix: '_r' }
  ],
  gloves: [
    { x: 0.209, y: 0.315, ax: 0.52, ay: 0.22, size: 0.265, rot: 0, z: 6, scaleY: 1.365, label: 'glove_l', splitSuffix: '_l' },
    { x: -0.195, y: 0.329, ax: 0.5, ay: 0.23, size: 0.228, rot: -1, z: 3, scaleX: 1.035, scaleY: 1.495, label: 'glove_r', splitSuffix: '_r' }
  ],
  shield: [{ x: -0.209, y: 0.447, ax: 0.59, ay: 0.54, size: 0.457, rot: 12, z: 4 }],
  weapon: [{ x: 0.063, y: 0.599, ax: 0.49, ay: 0.5, size: 0.570, rot: -58, z: 5, scaleX: 1.210, scaleY: 1.345 }],
  helmet: [{ x: 0.001, y: 0.066, ax: 0.5, ay: 0.5, size: 0.123, rot: 0, z: 5, scaleX: 0.970, scaleY: 1.045 }]
};
```

## Metadata
- **Job**: Warrior
- **Tier**: LV32
- **Race**: Arctron
- **Base Skin**: /assets/arctron/arctron_warrior.png
