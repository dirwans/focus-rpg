...
- **Animation Rig Lab Suite & Persistence (`public/animation-rig-lab.html`)**: Integrated `Save JSON`, `Import JSON`, 60 FPS `Record Video (.webm)`, and automatic LocalStorage persistence so calibration state is never lost on refresh.
- **Rig Lab Console Error & Backdrop Artifact Cleanup (`public/animation-rig-lab.html`)**: Removed stale `els.torsoBacking` style assignment causing `animation-rig-lab.html:608` console error, and removed rectangular backdrop box artifact.

### 🏗️ Milestone 192: HQ Screen Landscape Redesign, Cross-Faction Rollout, Auth Landscape Layout & Exit Portal [DEPLOYED] (2026-07-20)
- **New landscape HQ screen (`src/screens/HQScreen.jsx`, new file)**: replaces the portrait Main.jsx lobby for the "home" tab with a dedicated landscape layout — fixed-width nav sidebar (safe-area aware), header (CRD + race/job badge), a uniform-size NPC icon grid (68px circles, all 12 lobby NPCs, hover/press feedback driven by explicit React state rather than CSS `:hover` — proved unreliable/flickery on the real device this was tested against), a bottom deploy hatch (responsive-sized, two-tap-to-confirm on touch / hover-then-click on mouse — first tap only opens it and it stays open until either a second tap or mouse-leave, so a single tap can't accidentally deploy), and an ops-console FOCUS SESSION preview panel. NPC roster/positions ported from `public/ref/focus-rpg-design-aesthetic/project/HQ Screen - Arctron Directions.dc.html`, then re-laid-out into a responsive grid instead of the reference's fixed-scale scattered placement (which reintroduced the original landscape overlap bug on short real viewports — mitigated with a `max-height:480px` scale-down safety net, verified zero-overlap via live `getBoundingClientRect` checks, not just visual inspection).
- **Cross-faction rollout**: HQScreen generalized off `getFactionTheme(player.race)` (color) and `/assets/{race}/{race}_panel_bg.png` (background) instead of being hardcoded Arctron-only; `BottomNav.jsx` now routes all 3 races (not just Arctron) to `'hq'`, each keeping its own home label (HQ / MAINFRAME / SANCTUARY). Found and fixed a pre-existing color-source bug in `BottomNav.jsx` — it had its own out-of-sync `FACTION_PRIMARY` map (Bionex yellow, Celestra cyan) that disagreed with the canonical `factionThemes.js` (Bionex cyan, Celestra purple); now reads from the single canonical source.
- **Themed exit/logout portal**: sidebar button per-faction-labeled (Arctron "STAND DOWN", Bionex "DISCONNECT", Celestra "CLOSE THE WARD") that calls a full `useAuthStore.signOut()` (clears token + save, distinct from the hardware back button's plain `exitApp()`) then exits on native. Fixed a `Capacitor.isNativePlatform()` vs. truthy `window.Capacitor` check bug (the web shim object exists on web too, so the naive check always tried to call the native-only `exitApp()` on web and threw an uncaught `CapacitorException: Not implemented on web` — same bug fixed in both `HQScreen.jsx` and `App.jsx`'s hardware-backbutton handler). Sidebar restructured so the nav-item list scrolls in its own inner wrapper while the exit button stays in normal flow outside it — otherwise it could get pushed off-screen (needing a scroll to discover) once the sidebar had 7 items + the new exit button on short viewports.
- **Hardware back-button home-screen fix (`App.jsx`)**: the existing Capacitor `backButton` handler compared `state.screen !== 'main'` to decide "already home, ask to exit" — stale after HQScreen split home off into its own `'hq'` screen id, so pressing back from HQ used to detour through the old Main screen instead of going to the exit-confirm dialog. Now checks against `'hq'` (all races route there via BottomNav).
- **NpcModal back-navigation fix (`NpcModal.jsx`)**: new `hideLobby` prop — when opened from HQScreen's pillar icons (always straight into a specific NPC's view), the in-modal "back" arrow now closes the modal directly instead of surfacing the modal's own separate internal lobby list (a leftover from before HQScreen existed, now a confusing "why am I back at an old NPC list" duplicate of HQScreen's own pillar grid). The generic "VISIT NPC" entry point in `Main.jsx` is unaffected (still defaults `hideLobby=false`, in-modal lobby browsing preserved there).
- **Login screen landscape layout (`Auth.jsx`)**: was still rendering as a fixed 340px portrait card centered in the now-landscape `.game-container`, leaving ~225px of empty space on each side. Split into a two-column layout at `.game-container.landscape` (brand/logo column left, the actual login/register form column right, `.auth-card` widened to 720px) instead of just centering the narrow card.
- **Dead-code removal**: deleted an abandoned pre-flat-layer bone-rig prototype confirmed unused anywhere in the live app — `public/animation-lab-arctron.js` (orphaned loader, no HTML referenced it), `public/assets/arctron/{arctron_skeleton.json, arctron_texture.json, arctron_texture.png, test_bone_coords.cjs, test_bone_parser.cjs, test_bone_anims.cjs}`, and `src/components/{DragonBonesCharacter.jsx, DragonBonesPaperDoll.jsx}` (imported in `PilotSprites.jsx` but never actually rendered anywhere — dead import removed too). Deliberately kept `DragonBonesTest.jsx`, `pixi.js`, and `pixi-dragonbones-runtime` since `DragonBonesTest` is genuinely live-rendered in `SettingsModal.jsx` and reads a completely separate asset set (`/assets/dragonbones/mecha_1502b_*`), unaffected by the arctron-specific cleanup.
- **`.game-container` global border removal (`src/index.css`)**: removed the `2px solid #00e5ff` border + cyan glow `box-shadow` that wrapped every single screen/nav-tab (decorative "device bezel" look), per direct user feedback it was visually straining ("maraki ngelu").
- **Dev tooling**: `vite.config.js` given `host: true` so the local dev server is reachable over LAN (real-device testing without deploying); `.claude/launch.json` given a second `api` launch config (`npm start`, port 4001) alongside the existing `dev` config.
- **Hotfix (post-deploy, real-device report on the installed APK)**: the deploy hatch's fixed `340×90` sizing (matched 1:1 to the design reference's desktop mockup) looked oversized/heavy on an actual phone screen. Both dimensions switched to `clamp()` tied to viewport units (`width: clamp(190px, 34vw, 320px)`, `height: clamp(38px, 8vh, 66px)`, font/border-radius scaled proportionally too) instead of the fixed reference size, so it now shrinks gracefully on real device aspect ratios instead of staying pinned to the mockup's dimensions. Verified via live `getBoundingClientRect` at multiple viewports (832×430 → 283×38, 1024×600 → 320×48), no NPC-grid overlap regression.

### 🗡️ Milestone 191: Battle-Idle Rework — Per-Combo Files, Sword Orientation Fix, Asset Sync [DEPLOYED]
- **Naming convention correction**: replaced the generic dynamic `BattleIdleSprite.jsx` (auto-discovered assets by race/lineage/tier) with `src/components/lv1battlearctronwarrior.jsx` — sprite/gear components are strictly one dedicated file per faction-tier-job combo (`{tier}{purpose}{race}{lane}.jsx`), matching the existing `ArctronSprite`/`BionexSprite`/`CelestraSprite` split convention. `PilotSprites.jsx` routes to it with an explicit `race === 'arctron' && getJobLane(job) === 'warrior'` condition + a `fallback` prop (renders the existing static `GearOverlay`/`PaperDollStack` pose if this component's own assets 404). Each new combo needs its own new file + explicit import/branch, not a shared generic renderer.
- **Root-cause fix for "equipped weapon looks wrong" (`public/assets/weapons/defallfactionslv1sword{2,3,4}.png`)**: `resolveItemImage()` picks a pseudo-random variant among 4 tier-1 sword images; visual comparison showed `sword1.png` has hilt bottom-left/tip top-right while `sword2/3/4.png` were all authored 180° opposite. Any single calibration (in animation-lab.html or the pre-existing static `ArctronGearOverlay`, which almost certainly had this same latent bug already) could only be correct for one of the 4 possible appearances. Fixed at the asset level (rotated 3 files 180° via sharp) rather than in code — benefits every consumer of these images, not just the new battle sprite.
- **animation-lab.html**: added a "Project" name field used as the filename prefix for every export (Save/Export JSON/Spritesheet/Weapon-Shield), so exports are self-describing (`lv1-arctron-warrior-battle-base-back.png` etc.) instead of colliding generic names as more faction/tier/job combos get authored. `applyToAllFrames()` fixed to also clone the layer's image + an independently-copied mask (not just x/y/scale/rotation) — swapping a layer's source image or touching up its erase-transparency on one frame now actually propagates when applied to the rest, instead of only moving the old image's position.
- **Data/asset sync fix**: found `weapon_shield_layers.json` and `spritesheet_fixed.png` in `Battle-Sprites-lv1-arctronwarrior/` had drifted out of sync (JSON described a 4683×2652×5-frame sheet, actual PNG was 3666×695×6-frame) — almost certainly from parallel manual edits. Re-derived both fresh from the latest known-good animation-lab project state (`sprite_studio_project.json`, confirmed to have the grip-erase transparency work intact) via the same validated export pipeline, producing a consistent 2500×316×5-frame pair. Verified by direct dimension check (PNG width == JSON frameWidth × frameCount) before deploying.

### 🗡️ Milestone 190: Arctron Warrior Battle-Idle Animation (Tier 1) [DEPLOYED]
- **Authoring tool overhaul (`public/animation-lab.html`)**: matured into a real sprite-sheet factory over this session — layer roles (Base/Weapon/Shield) with a front/back "grip sandwich" so weapon/shield swap independently of the baked body art; auto-snap calibration (Copy From Previous Frame, Apply To All Frames, Keyframe + Interpolate for non-linear motion like a breathing loop); an Asset Library panel (folder drill-down + global search) backed by a new `GET /api/asset-library` route (`server.js`) so gear pieces load by click instead of the OS file picker; Undo/Ctrl+Z with a session-guarded snapshot stack; an Eraser Opacity slider (soft/partial erase, not just a hard cut); manual number-entry paired with every drag slider. Fixed several pre-existing bugs surfaced along the way: `downloadSpritesheet()`'s cell-centering math was clipping ~75% of every exported frame into one corner; the Eraser's click-to-mask-pixel math assumed a horizontal multi-frame strip layout that never matched what was actually drawn, so erasing silently hit the wrong pixels; the Live Preview recomputed canvas size per-frame independently instead of from a shared bounding box, causing visible bobbing during playback.
- **New runtime component (`src/components/ArctronBattleIdleSprite.jsx`)**: canvas-based 5-frame idle-breathing loop for Arctron Warrior tier 1, reading `public/assets/arctron/battle/warrior_lv1_idle/base_back.png` (flattened body spritesheet) + `weapon_shield_layers.json` (per-frame pixel x/y/scale/rotation). Composites the player's actually-equipped weapon/shield (resolved via `resolveItemImage`) on top each frame, normalized by image-width ratio against the calibration reference art so a differently-sized equipped item still lands at the right on-screen size.
- **Wiring (`src/components/PilotSprites.jsx`)**: `PilotSprite` now routes the logged-in Arctron player to `ArctronBattleIdleSprite` when `isBattle && getJobLane(job) === 'warrior'`; every other race/lane/tier is unchanged (falls through to the existing static `ArctronGearOverlay` pose) since only tier1 Warrior has an authored export so far.
- **Verified live** in the actual Deploy Unit → fight flow (`Main.jsx`): canvas renders, frames visibly cycle, and the equipped weapon (a Lv.1 Greatsword — a different image than the calibration reference sword) correctly swapped in at the calibrated position; no-shield-equipped case renders cleanly with nothing extra drawn (no crash).
...
- **Ascension UI Overhaul (src/screens/Ascension.jsx)**:
  - Rebuilt Bionex evolution tree into tabbed layout with Blueprint Loadouts.
  - Fixed CSS filtering and restored uncropped Defender Lv42 image.
...
### ⚙️ Milestone 151: Arctron ARES Siege Kit Shop & Assembly Mechanics [DEPLOYED]
- **A.R.E.S. Siege Kit Material Purchasing (`src/store/gameStore.js` & `src/screens/Ascension.jsx`)**:
  - Implemented the actual in-game purchasing of the individual **Siege Kit items** per level in the A.R.E.S. ASSEMBLY SHOP tab:
    - **Lv.32** — **Siege Kit** (`5,000,000 CRD`)
    - **Lv.42** — **Advanced Siege Kit** (`100,000,000 CRD`)
    - **Lv.55** — **Shining Siege Kit** (`500,000,000 CRD`)
    - **Lv.65** — **Adv. Shining Siege Kit** (`2,000,000,000 CRD` - Locked)
  - Created `buySiegeKit()` store action to deduct the corresponding CRD cost and store the acquired item ID into the player's new persistent inventory state: `player.ownedSiegeKits`.
- **Hangar Assembly Requirements check (`src/store/gameStore.js` & `src/screens/Ascension.jsx`)**:
...
### Milestone 166: AuditorRoom Material Card Bottom Strip Fix [DEPLOYED]
...
### Milestone 167: Excel Import/Export System di AuditorRoom [DEPLOYED]
...
### Milestone 168: main.jsx BASE Tab Design Alignment ke unit.jsx Style [DEPLOYED]
...
### Milestone 169: NpcModal Sprite Fix + Unit.jsx Temporal Dead Zone Bug Fix [DEPLOYED]
...
### Milestone 170: SPRITE_CALIBRATIONS Refactor ke File Terpisah [DEPLOYED]
...
### Milestone 171: Unit.jsx Celestra Mage Path Tab Overflow Fix [DEPLOYED]
...
### Milestone 174: Material Ore & Shard Background Removal + Cache Bump [DEPLOYED]
...
- **Problem 2 (legendary size + hidden text)**: Legendary ore backup (2263×1446) had "Legendary / L" text watermark baked into the right side of the image (gold text on black bg). BFS kept text opaque (gold ≠ black), extending bounding box width → fit-within-320 only scaled to 320×206. Text also remained visible as floating gold letters.
- **Fix**: After BFS, apply connected-component filter (`scipy.ndimage.label`): keep only the largest opaque component (ore body = 1,730,624 px = 99%), remove isolated components (text letters: 14,944 px + 3,161 px + tiny orphans). Without text bloating bbox, ore rescales to 320×257 — same height range as other ores. No cropping needed.
...
- **WebGL/Mesh Polyfill**: Added a compatibility polyfill in [index.html](file:///c:/projects/focus-rpg/index.html) to map `PIXI.mesh.Mesh.DRAW_MODES` to `PIXI.DRAW_MODES` (with `TRIANGLES: 4`), resolving a runtime `Uncaught TypeError` in `dragonBones.js` under PixiJS v5.
- **2D Paper Doll System Revert**: Reverted active pilot character rendering in [PilotSprites.jsx](file:///c:/projects/focus-rpg/src/components/PilotSprites.jsx) from DragonBones back to the flat 2D `PaperDollStack` overlay system. Added `RACE_DEFAULTS` directly in the component file to cleanly align weapons and shields (e.g. for `arctron` at `top: 38%, left: 5%` for weapon and `top: 32%, left: 55%` for shield) without requiring external calibration files.
- **Arctron Layer Filtering**: Restricted `PaperDollStack` to only render `weapon` and `shield` slots when the player's race is `arctron`, preventing standalone mecha item icons (such as helmet or chest plates) from rendering as stretched, oversized full-body overlays on top of the base mecha portrait.
